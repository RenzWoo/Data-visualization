...
```{r}
us_hex <- st_read("3D POPULATION/kontur_population_US_20231101.gpkg") %>% st_transform(3857)
us_admin <- st_read("3D POPULATION/kontur_boundaries_US_20230628.gpkg") %>% st_transform(3857) 
```
...
``` {r}
distinct_names <- us_admin %>% distinct(name_en)
print(distinct_names)
```

``` {r}
us_admin <- us_admin %>% filter(!name_en %in% c(
...
```{r}
us_admin_simplified <- st_simplify(us_admin, dTolerance = 1000)  # Speed up intersection
us_hex <- st_filter(us_hex, us_admin_simplified)
```

``` {r}
# Creating BD Boundary
us_boundary <- us_admin %>%
  st_geometry %>%
  st_union %>%
  st_sf %>%
  st_make_valid()
...
ggplot(us_hex) + 
  ...
  geom_sf(data = us_boundary, fill = NA, color = "black")
```
